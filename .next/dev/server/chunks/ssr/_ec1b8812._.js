...
function ParentDashboard() {
    ...
    const fetchChildStats = async (childId)=>{
        try {
            const { data: expenses } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('expenses').select('amount, type').eq('user_id', childId);
            const { data: tasks } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('tasks').select('status').eq('user_id', childId);
            const { count: meals } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('meals').select('*', {
                count: 'exact',
                head: true
            }).eq('user_id', childId).gte('timestamp', new Date().toISOString().split('T')[0]);
            const totalExp = expenses?.filter((e)=>e.type === 'expense').reduce((sum, e)=>sum + Number(e.amount), 0) || 0;
            const completed = tasks?.filter((t)=>t.status === 'done').length || 0;
            setChildStats({
                totalExpenses: totalExp,
                activeTasks: (tasks?.length || 0) - completed,
...
//# sourceMappingURL=_ec1b8812._.js.map